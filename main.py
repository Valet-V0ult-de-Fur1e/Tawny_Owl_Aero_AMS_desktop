...
from PyQt6.QtCore import Qt, QTimer, pyqtSignal, QObject, QDateTime, QThread
...
#         if self.camera_mode == 'video':
#             self.init_video_controls(main_layout)
#         else:
#             self.init_photo_controls(main_layout)

#         nav_layout = QHBoxLayout()
#         back_btn = QPushButton("< Назад")
#         back_btn.clicked.connect(lambda: self.window.navigate_to(MainPage))
#         finish_btn = QPushButton("Завершить осмотр")
#         finish_btn.clicked.connect(self.cleanup)
#         nav_layout.addWidget(back_btn)
#         nav_layout.addStretch()
#         nav_layout.addWidget(finish_btn)
#         main_layout.addLayout(nav_layout)

#     def init_video_controls(self, layout):
#         btn_style = """
#             QPushButton {
#                 padding: 10px 20px;
#                 font-size: 14px;
...
#         self.record_btn.setStyleSheet(btn_style + "background-color: #27ae60; color: white;")
#         self.record_btn.clicked.connect(self.start_recording)

#         self.pause_btn = QPushButton("Пауза")
#         self.pause_btn.setStyleSheet(btn_style + "background-color: #f39c12; color: black;")
#         self.pause_btn.clicked.connect(self.toggle_pause)
#         self.pause_btn.setEnabled(False)

#         self.stop_btn = QPushButton("Стоп")
#         self.stop_btn.setStyleSheet(btn_style + "background-color: #e74c3c; color: white;")
...
#     def init_photo_controls(self, layout):
...
#             filename = f"video_{timestamp}_cam{cam_id}.mp4"
...
#             worker.frame_ready.connect(self.process_frame)
...
#         status = "Пауза" if worker.paused else "Идет запись..."
#         self.status_label.setText(status)
#         self.pause_btn.setText("Возобновить" if worker.paused else "Пауза")
...
#                 ret, frame = cap.read()
#                 if ret:
#                     filename = f"photo_{timestamp}_cam{cam_id}.jpg"
#                     path = os.path.join("photos", filename)
#                     cv2.imwrite(path, frame)
#                     photo_data.append({"camera_id": cam_id, "path": path})
#                 cap.release()
#             except Exception as e:
#                 print(f"Photo error cam {cam_id}: {str(e)}")
        
#         with open(os.path.join("photos", "metadata.json"), 'w') as f:
#             json.dump(photo_data, f)
            
#         QMessageBox.information(self, "Успех", f"Сохранено фото: {len(photo_data)}")

#     def on_worker_finished(self):
#         if all(not worker.isRunning() for worker in self.workers):
#             self.cleanup()
#             QMessageBox.information(self, "Завершено", "Видео успешно сохранено")
...
class CameraWorker(QThread):
    frame_ready = pyqtSignal(int, any)  # Сигнал для передачи кадра (ID камеры, кадр)
    finished = pyqtSignal()  # Сигнал, когда работа завершена
    ...
    def run(self):
        ...
        while self.running:
            if not self.paused:
                ...
                self.msleep(100)  # Ждем немного, если находимся на паузе
        cap.release()
        self.finished.emit()
...
class ShootingControlPage(QWidget):
    ...
    def start_recording(self):
        ...
        for cam_id in self.cameras:
            ...
            worker = CameraWorker(cam_id)
            worker.frame_ready.connect(lambda cam_id=cam_id: self.process_frame(cam_id))
            worker.finished.connect(self.on_worker_finished)
            worker.start()
            self.workers.append(worker)
    ...
    def toggle_pause(self):
        for worker in self.workers:
            worker.paused = not worker.paused
        status = "Пауза" if self.workers[0].paused else "Идет запись..."
        self.status_label.setText(status)
        self.pause_btn.setText("Возобновить" if self.workers[0].paused else "Пауза")
